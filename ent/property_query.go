...
func (pq *PropertyQuery) QueryOwner() *UserQuery {
...
// Returns a *NotSingularError when exactly one Property entity is not found.
...
// Returns a *NotSingularError when exactly one Property ID is not found.
...
func (pq *PropertyQuery) WithOwner(opts ...func(*UserQuery)) *PropertyQuery {
	query := &UserQuery{config: pq.config}
...
func (pq *PropertyQuery) sqlAll(ctx context.Context) ([]*Property, error) {
	...
	if query := pq.withOwner; query != nil {
		...
		for i := range nodes {
			if nodes[i].user_properties == nil {
				continue
			}
			fk := *nodes[i].user_properties
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_properties" returned %v`, n.ID)
			}
			for i := range nodes {
