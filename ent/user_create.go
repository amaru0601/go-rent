...
func (uc *UserCreate) check() error {
	...
	if v, ok := uc.mutation.Names(); ok {
		if err := user.NamesValidator(v); err != nil {
			return &ValidationError{Name: "names", err: fmt.Errorf(`ent: validator failed for field "names": %w`, err)}
		}
	}
	if _, ok := uc.mutation.Lastnames(); !ok {
		return &ValidationError{Name: "lastnames", err: errors.New(`ent: missing required field "lastnames"`)}
	}
	if v, ok := uc.mutation.Lastnames(); ok {
		if err := user.LastnamesValidator(v); err != nil {
			return &ValidationError{Name: "lastnames", err: fmt.Errorf(`ent: validator failed for field "lastnames": %w`, err)}
		}
	}
